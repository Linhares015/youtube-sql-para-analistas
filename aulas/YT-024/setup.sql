...
CREATE TABLE pedidos_desempenho (
...
INSERT INTO pedidos_desempenho
SELECT id,
       1 + ((id * 37) % 120),
       DATE '2024-01-01' + ((id * 17) % 730),
       (ARRAY['pago', 'enviado', 'entregue', 'cancelado'])[1 + (id % 4)],
       round((50 + (id % 5000) * 0.73)::numeric, 2)
FROM generate_series(1, 120000) AS g(id);

ANALYZE vendas.pedidos_desempenho;
